...
def save_txt_file(list_txt):
    ...
    with open(path_to_save, mode="a+") as f:
        #in case I receive just a tring, append it
        f.write(list_txt+'\n')
        #in case I receive a list of texts
...
for indx in range(len(list_chunks)):
    ...
    audio_clip.close()
    # #Using whisper, move out of here, as the model is created multiple times
    # try:
    ...
    # print(out)
    # total_text.append(out['text'])
    # list_of_texts= out['segments']
    # for line in list_of_texts:
    #     line['start']+=start
    #     line['end']+= start
    #     line['id']=id_encounter
    #     id_encounter+=1
    #     end=line['end']
